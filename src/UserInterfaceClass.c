...
int32_t UI_GetReg(CUI this, MC_Protocol_REG_t bRegID)
{
  ...
  case MC_PROTOCOL_REG_ABS_MEC_ANGLE:
  	  {
  		  bRetVal = SPD_GetAbsMecAngle(MCT_GetSpeednPosSensorMain(oMCT));
  	  }
  	  break;

  case MC_PROTOCOL_REG_POS_REF:
	  {
		  bRetVal = STC_GetPosRef(MCT_GetSpeednTorqueController(oMCT));
	  }
	  break;

  case MC_PROTOCOL_REG_STM_STATE:
	  {
		  bRetVal = (int32_t) MCI_GetSTMState(oMCI);
	  }
	  break;
