/**
  ...
  *
  ******************************************************************************
  *Rmq : plus besoin d'inclure la library FrameCom
  */

/* Includes ------------------------------------------------------------------*/
#include "../inc/MotorControlProtocolClass.h"

#include "../inc/MotorControlProtocolPrivate.h"
#include "../boob/MotorControlProtocolPrivate.h"
#include "../inc/MotorControlProtocolPrivate.h"
#include "../inc/struct_can_msg.h"
#include "../inc/UserInterfaceClass.h"
#include "../inc/UserInterfacePrivate.h"
/* Private define ------------------------------------------------------------*/
...
static CMCP_UI _mcinstance;
typedef void (*can_sender) (uint8_t,uint8_t *,uint8_t);
static can_sender Frame_Send;
void MCP_custom_Recive(void* can_msg);
...
void MCP_WaitNextFrame(CMCP_UI this)
{
 /* _DCMCP oMCP = (_DCMCP)(((_CUI)this)->DerivedClass);
  ...
  Frame_Receive(MC_NULL, oMCP->DVars_str.BufferFrame, &(oMCP->DVars_str.BufferSize));
*/
	}
...
void MCP_ReceivedFrame(CMCP_UI this, uint8_t Code, uint8_t *buffer, uint8_t Size)
{
  ...
  case MC_PROTOCOL_CODE_GET_REG:
    {
      ...
      case MC_PROTOCOL_REG_SC_COMPLETED:
        {
          ...
          {
            Frame_Send(K_NOERROR, (uint8_t*)(&value), 1);
            bNoError = TRUE;
            RequireAck = FALSE;
          }
        }
      ...
      {
    	  int16_t value = UI_GetReg((CUI)this, MC_PROTOCOL_REG_I_Q_REF);
          /* 16bit variables */
          Frame_Send(K_NOERROR, (uint8_t*)(&value), 2);
          bNoError = TRUE;
          RequireAck = FALSE;
      }
      break;
      ...
      {
    	  int16_t value = UI_GetReg((CUI)this, MC_PROTOCOL_REG_I_Q);
          /* 16bit variables */
          Frame_Send(K_NOERROR, (uint8_t*)(&value), 2);
          bNoError = TRUE;
          RequireAck = FALSE;
      ...
      {
    	  int16_t value = UI_GetReg((CUI)this, MC_PROTOCOL_REG_SPEED_REF);
          //int16_t value =-10;
    	  /* 16bit variables */
          Frame_Send(ACK_NOERROR, (uint8_t*)(&value), 2);
          bNoError = TRUE;
          RequireAck = FALSE;
      }
      break;
      ...
      {
    	  int16_t value = UI_GetReg((CUI)this, MC_PROTOCOL_REG_TORQUE_REF);
          //int16_t value =-10;
    	  /* 16bit variables */
          Frame_Send(ACK_NOERROR, (uint8_t*)(&value), 2);
          bNoError = TRUE;
      ...
      case MC_PROTOCOL_REG_PFC_I_KP:
      {
    	  uint16_t value = UI_GetReg((CUI)this, MC_PROTOCOL_REG_CONTROL_MODE);
          /* 16bit variables */
          Frame_Send(ACK_NOERROR, (uint8_t*)(&value), 2);
          bNoError = TRUE;
          RequireAck = FALSE;
      ...
      case MC_PROTOCOL_REG_PFC_I_KI:
      {
    	  uint16_t value = UI_GetReg((CUI)this, MC_PROTOCOL_REG_STM_STATE);
          /* 16bit variables */
          Frame_Send(ACK_NOERROR, (uint8_t*)(&value), 2);
          bNoError = TRUE;
          RequireAck = FALSE;
      ...
      case MC_PROTOCOL_REG_PFC_I_KD:
      {
    	  uint16_t value = UI_GetReg((CUI)this, MC_PROTOCOL_REG_SPEED_KP);
          //int16_t value =-10;
    	  /* 16bit variables */
          Frame_Send(ACK_NOERROR, (uint8_t*)(&value), 2);
          bNoError = TRUE;
      ...
      case MC_PROTOCOL_REG_PFC_V_KP:
      {
    	  uint16_t value = UI_GetReg((CUI)this, MC_PROTOCOL_REG_SPEED_KI);
          //int16_t value =-10;
    	  /* 16bit variables */
          Frame_Send(ACK_NOERROR, (uint8_t*)(&value), 2);
          bNoError = TRUE;
      ...
      case MC_PROTOCOL_REG_PFC_V_KI:
      {
    	  uint16_t value = UI_GetReg((CUI)this, MC_PROTOCOL_REG_TORQUE_KP);
          //int16_t value =-10;
    	  /* 16bit variables */
          Frame_Send(ACK_NOERROR, (uint8_t*)(&value), 2);
          bNoError = TRUE;
      ...
      case MC_PROTOCOL_REG_PFC_V_KD:
      {
    	  uint16_t value = UI_GetReg((CUI)this, MC_PROTOCOL_REG_TORQUE_KI);
          //int16_t value =-10;
    	  /* 16bit variables */
          Frame_Send(ACK_NOERROR, (uint8_t*)(&value), 2);
          bNoError = TRUE;
      ...
      case MC_PROTOCOL_REG_FF_1Q:
      {
    	  int32_t value = UI_GetReg((CUI)this, MC_PROTOCOL_REG_ABS_MEC_ANGLE);
          /* 32bit variables */
          Frame_Send(ACK_NOERROR, (uint8_t*)(&value), 4);
          bNoError = TRUE;
          RequireAck = FALSE;
          break;
      }
      ...
      case MC_PROTOCOL_REG_FF_1D:
      {
    	  ...
    	  int32_t value = (int32_t) UI_GetReg((CUI)this, MC_PROTOCOL_REG_SPEED_MEAS);
          /* 32bit variables */
          Frame_Send(ACK_NOERROR, (uint8_t*)(&value), 4);
          bNoError = TRUE;
          RequireAck = FALSE;
      ...
      {
    	  //todo : remove this, used for debug
    	  int32_t value = UI_GetReg((CUI)this, MC_PROTOCOL_REG_POS_REF);
          /* 32bit variables */
          Frame_Send(ACK_NOERROR, (uint8_t*)(&value), 4);
          bNoError = TRUE;
          RequireAck = FALSE;
  ...
  case MC_PROTOCOL_CODE_GET_BOARD_INFO:
    {
      ...
      Frame_Send(ACK_NOERROR, outBuff, 32);
      bNoError = TRUE;
    }
    break;
  ...
  case MC_PROTOCOL_CODE_GET_REVUP_DATA:
    {
      ...
      Frame_Send(ACK_NOERROR, outBuff, 8);
    }
    break;
  ...
  case MC_PROTOCOL_CODE_GET_MP_INFO:
    {
      ...
      if (bNoError)
      {
        Frame_Send(ACK_NOERROR, MPInfo.data, MPInfo.len);
        RequireAck = FALSE;
      }
    }
    break;
  ...
  if (RequireAck)
  {
    if (bNoError)
    {
      Frame_Send(ACK_NOERROR, MC_NULL, 0);
    }
    else
    {
      Frame_Send(ACK_ERROR, &bErrorCode, 1);
    }
  }
}
...
void MCP_SendTimeoutMeassage(CMCP_UI this)
{
  ...
  Frame_Send(K_ERROR, &bErrorCode, 1);
}
...
void MCP_SendATRMeassage(CMCP_UI this)
{
  ...
  *(uint32_t*)(&buff[0]) = wUID;
  ...
  Frame_Send(ATR_FRAME_START, buff, 13);
}
/*
 * Est inisialiser avec une reference a &SendMCInterface
 */
void MCP_custom_init(CMCP_UI this,void (*sender)(uint8_t,uint8_t *,uint8_t)){
	if(isinit != 0xFF){
		isinit = 0xFF;
		_mcinstance = this;
		Frame_Send = (*sender);
...
void MCP_custom_Recive(void* can_msg){
	if(isinit==0xFF && ((CAN_Msg)can_msg)->valide){
		if(((CAN_Msg)can_msg)->size){
			MCP_ReceivedFrame(_mcinstance, ((CAN_Msg)can_msg)->message[0], ((CAN_Msg)can_msg)->message+1, ((CAN_Msg)can_msg)->size - 1u);
		}
	}
}
