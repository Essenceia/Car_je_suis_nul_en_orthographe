/**
 * CUSTOM MC TYPE
 *Todo: utliser et ne pas oublier d'inclure dans le projet finale
  ******************************************************************************
  * @file    MC_type.h
...
/**
  * @brief Used to specfy the state of the ongoing trapezoidal speed profile
  */
typedef enum{
	TS_CST_ACC_P, //First constant acceleration state
	TS_CST_SPD,// Constant speed state
	TS_CST_ACC_N //First constant acceleration state
} STC_TrapezoidalState_t;

/**
  * @brief Used to specfy the form of the ongoing trapezoidal speedprofile, it can be either normal degenerate or none.
  * 	   This value is computed when a speed ramp is executed in position mode, depending on given speeds accelerations and total distance
  *		   NONE value means there is no trapezoidal speed profile being followed.
  */
...
#define OPT_COM     0x00 /*Nous avons voler le code de com pour y mettre can*/
#define OPT_LCD     0x01 /*!<Bit field indicating that the UI uses LCD manager.*/
#define OPT_CAN     0x02 /*!<Bit field indicating that the UI uses can communication. Todo : implementer partout ou il est appeller */
